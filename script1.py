...
conn_str = (
    r'DRIVER={SQL Server};'
    r'PORT=1433;'
    r'SERVER=LAPTOP-MUFDTM9A\SQLEXPRESS02;'
    r'DATABASE=practice;'
...
def extract():
    try:
        ...
        s = session()
        #execute query
        src_tables = s.execute("""select t.name as table_name from sys.tables t where t.name in ('salesman','customer','ORDERS')""")
...
def load(df,tbl):
    try:
        ...
        #save to s3
        upload_file_bucket = 'elt-snfk/dela/order/order.csv'
