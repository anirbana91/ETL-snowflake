...
conn_str = (
    r'DRIVER={SQL Server};'
    r'PORT=1433;'
    r'SERVER=DESKTOP-S4JRKES;'
    r'DATABASE=Projects_sql;'
...
def extract():
    try:
        ...
        #execute query
        src_tables = s.execute("""select t.name as table_name from sys.tables t where t.name in ('salesman1','customer','orders')""")
...
def load(df,tbl):
    try:
        ...
        #save to s3
        upload_file_bucket = 'transfer-s3'
